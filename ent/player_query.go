...
type PlayerQuery struct {
	...
	withInviter  *PlayerQuery
	withInvitees *PlayerQuery
...
func (pq *PlayerQuery) QueryInviter() *PlayerQuery {
	query := (&PlayerClient{config: pq.config}).Query()
...
func (pq *PlayerQuery) QueryInvitees() *PlayerQuery {
	query := (&PlayerClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (pq *PlayerQuery) Clone() *PlayerQuery {
	...
	return &PlayerQuery{
		...
		withInviter:  pq.withInviter.Clone(),
		withInvitees: pq.withInvitees.Clone(),
...
func (pq *PlayerQuery) WithInviter(opts ...func(*PlayerQuery)) *PlayerQuery {
	query := (&PlayerClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withInviter = query
	return pq
}
...
func (pq *PlayerQuery) WithInvitees(opts ...func(*PlayerQuery)) *PlayerQuery {
	query := (&PlayerClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withInvitees = query
	return pq
}
...
func (pq *PlayerQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Player, error) {
	...
	if query := pq.withInviter; query != nil {
		if err := pq.loadInviter(ctx, query, nodes, nil,
			func(n *Player, e *Player) { n.Edges.Inviter = e }); err != nil {
			return nil, err
		}
	}
	if query := pq.withInvitees; query != nil {
		if err := pq.loadInvitees(ctx, query, nodes,
			func(n *Player) { n.Edges.Invitees = []*Player{} },
			func(n *Player, e *Player) { n.Edges.Invitees = append(n.Edges.Invitees, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PlayerQuery) loadInviter(ctx context.Context, query *PlayerQuery, nodes []*Player, init func(*Player), assign func(*Player, *Player)) error {
...
func (pq *PlayerQuery) loadInvitees(ctx context.Context, query *PlayerQuery, nodes []*Player, init func(*Player), assign func(*Player, *Player)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(player.FieldInviterID)
	}
	query.Where(predicate.Player(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(player.InviteesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (pq *PlayerQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := pq.ctx.Fields; len(fields) > 0 {
		...
		if pq.withInviter != nil {
			_spec.Node.AddColumnOnce(player.FieldInviterID)
		}
	}
