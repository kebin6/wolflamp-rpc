...
func (fq *FileQuery) QueryBanner() *BannerQuery {
	query := (&BannerClient{config: fq.config}).Query()
...
func (fq *FileQuery) Clone() *FileQuery {
	...
	return &FileQuery{
		...
		withBanner: fq.withBanner.Clone(),
...
func (fq *FileQuery) WithBanner(opts ...func(*BannerQuery)) *FileQuery {
	query := (&BannerClient{config: fq.config}).Query()
...
func (fq *FileQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*File, error) {
	...
	if query := fq.withBanner; query != nil {
		if err := fq.loadBanner(ctx, query, nodes, nil,
			func(n *File, e *Banner) { n.Edges.Banner = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (fq *FileQuery) loadBanner(ctx context.Context, query *BannerQuery, nodes []*File, init func(*File), assign func(*File, *Banner)) error {
	...
	query.withFKs = true
	query.Where(predicate.Banner(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(file.BannerColumn), fks...))
	}))
