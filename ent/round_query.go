...
type RoundQuery struct {
	...
	withFold   *RoundLambFoldQuery
	withInvest *RoundInvestQuery
...
// QueryFold chains the current query on the "fold" edge.
func (rq *RoundQuery) QueryFold() *RoundLambFoldQuery {
	query := (&RoundLambFoldClient{config: rq.config}).Query()
...
// QueryInvest chains the current query on the "invest" edge.
func (rq *RoundQuery) QueryInvest() *RoundInvestQuery {
	query := (&RoundInvestClient{config: rq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (rq *RoundQuery) FirstID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (rq *RoundQuery) FirstIDX(ctx context.Context) uint64 {
...
func (rq *RoundQuery) OnlyID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (rq *RoundQuery) OnlyIDX(ctx context.Context) uint64 {
...
func (rq *RoundQuery) IDs(ctx context.Context) (ids []uint64, err error) {
...
func (rq *RoundQuery) IDsX(ctx context.Context) []uint64 {
	ids, err := rq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (rq *RoundQuery) Clone() *RoundQuery {
	...
	return &RoundQuery{
		...
		withFold:   rq.withFold.Clone(),
		withInvest: rq.withInvest.Clone(),
...
// WithFold tells the query-builder to eager-load the nodes that are connected to
// the "fold" edge. The optional arguments are used to configure the query builder of the edge.
func (rq *RoundQuery) WithFold(opts ...func(*RoundLambFoldQuery)) *RoundQuery {
	query := (&RoundLambFoldClient{config: rq.config}).Query()
...
// WithInvest tells the query-builder to eager-load the nodes that are connected to
// the "invest" edge. The optional arguments are used to configure the query builder of the edge.
func (rq *RoundQuery) WithInvest(opts ...func(*RoundInvestQuery)) *RoundQuery {
	query := (&RoundInvestClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (rq *RoundQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Round, error) {
	...
	if query := rq.withFold; query != nil {
		if err := rq.loadFold(ctx, query, nodes,
			func(n *Round) { n.Edges.Fold = []*RoundLambFold{} },
			func(n *Round, e *RoundLambFold) { n.Edges.Fold = append(n.Edges.Fold, e) }); err != nil {
			return nil, err
		}
	}
	if query := rq.withInvest; query != nil {
		if err := rq.loadInvest(ctx, query, nodes,
			func(n *Round) { n.Edges.Invest = []*RoundInvest{} },
			func(n *Round, e *RoundInvest) { n.Edges.Invest = append(n.Edges.Invest, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *RoundQuery) loadFold(ctx context.Context, query *RoundLambFoldQuery, nodes []*Round, init func(*Round), assign func(*Round, *RoundLambFold)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint64]*Round)
	for i := range nodes {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(roundlambfold.FieldRoundID)
	}
	query.Where(predicate.RoundLambFold(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(round.FoldColumn), fks...))
	}))
...
func (rq *RoundQuery) loadInvest(ctx context.Context, query *RoundInvestQuery, nodes []*Round, init func(*Round), assign func(*Round, *RoundInvest)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint64]*Round)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(roundinvest.FieldRoundID)
	}
	query.Where(predicate.RoundInvest(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(round.InvestColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (rq *RoundQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(round.Table, round.Columns, sqlgraph.NewFieldSpec(round.FieldID, field.TypeUint64))
	_spec.From = rq.sql
